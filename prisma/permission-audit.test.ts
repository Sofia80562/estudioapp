...
describe('extractRequiredPermissionCodes (estudioapp)', () => {
...
describe('diffPermissionCatalog', () => {
	...
	it('detecta permisos huérfanos frente al conjunto real exigido por el código', () => {
		const { requiredCodes } = extractRequiredPermissionCodes(PAGES_API_DIR);
		const seededCatalogCodes = new Set([...requiredCodes, 'legacy.read', 'legacy.manage']);

		const { orphanCodes, missingCodes } = diffPermissionCatalog(requiredCodes, seededCatalogCodes);

		expect(orphanCodes.sort()).toEqual(['legacy.manage', 'legacy.read']);
