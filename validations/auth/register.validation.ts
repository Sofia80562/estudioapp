...
export const registerSchema = z
	.object({
		...
		lastName: z
			...
			.min(1, VALIDATION_MESSAGES.REQUIRED)
			.max(100, VALIDATION_MESSAGES.MAX_LENGTH(100)),
		accountType: z.enum(['estudiante', 'gestor-academico']),
		organization: organizationInputSchema.optional(),
		venue: venueInputSchema.optional(),
	})
	.superRefine((data, ctx) => {
		if (data.accountType !== 'gestor-academico') {
			return;
		}
		...
		if (!data.organization) {
			ctx.addIssue({
				code: 'custom',
				path: ['organization'],
				message: 'Los datos de la institución son obligatorios para este tipo de cuenta.',
			});
		}
		...
		if (!data.venue) {
			ctx.addIssue({
				code: 'custom',
				path: ['venue'],
				message: 'Los datos del campus son obligatorios para este tipo de cuenta.',
			});
		}
	});
