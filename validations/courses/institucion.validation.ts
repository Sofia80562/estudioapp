...
export const createOrganizationSchema = z
	.object({
		name: z.string().min(1, 'Institution name is required').max(150),
...
export const updateOrganizationSchema = z
	.object({
		name: z.string().min(1, 'Institution name is required').max(150).optional(),
		legalName: z.string().max(200).optional(),
		taxIdentification: z.string().max(30).optional(),
		email: z.string().email('Invalid email format').optional().or(z.literal('')),
		phone: z.string().max(20).optional().or(z.literal('')),
		domain: z.string().max(255).optional().or(z.literal('')),
		// Exclusivo de Administrador global; nunca acepta 'PENDING_APPROVAL'.
...
export const organizationParamsSchema = z
	.object({
		organizationId: z.string().uuid('Invalid institution ID'),
	})
	.strict();
