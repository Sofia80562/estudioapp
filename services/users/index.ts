...
const validateAssignableRoles = async (
    ...
    if (roles.some((role: { organizationId: string | null }) => role.organizationId !== null && !organizationId)) {
        throw new BusinessRuleError('La organización es obligatoria para los roles de organización.');
    }

    if (
        organizationId &&
        roles.some((role: { organizationId: string | null }) => role.organizationId !== null && role.organizationId !== organizationId)
    ) {
...
export const create = async (body: CreateUserBody, actingUser: SessionUser) => {
    ...
    return {
        ...
        roles: userWithRoles.userRoles.map((ur: { role: any }) => ur.role),
...
export const getById = async (userId: string) => {
    ...
    return {
        ...
        lastName: user.profile?.lastName ?? '',
        active: user.status === 'ACTIVE',
        roles: user.userRoles.map((ur: { role: any }) => ur.role),
        createdAt: user.createdAt,
        updatedAt: user.updatedAt,
    };
};
...
export const updateAdminProfile = async (
    ...
    const result = (await userData.updateAdminProfile(userId, body, isAdministrator(actingUser))) as any;
