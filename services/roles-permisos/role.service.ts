...
const ensurePermissionSubset = (actingUser: SessionUser, permissionCodes: string[]): void => {
    ...
    const grantedCodes = new Set(actingUser.permissions.map((permission: { code: string }) => permission.code));
...
const createRole = async (
    ...
    try {
        return await roleDb.withTransaction(async (repository: RoleTransactionRepository) => {
            const permissions = await getPermissionsOrThrow(repository, input.permissionIds);
            ensurePermissionSubset(
                actingUser,
                permissions.map((permission: { code: string }) => permission.code),
            );
            ...
            await repository.writeAudit({
                ...
                changes: {
                    name: input.name,
                    description: input.description ?? null,
                    permissionsAdded: permissions.map((permission: { code: string }) => permission.code),
                },
            });
...
const updateRole = async (
    ...
    try {
        return await roleDb.withTransaction(async (repository: RoleTransactionRepository) => {
            ...
            const currentPermissions = current.permissions.map(({ permission }: { permission: { code: string; id: string } }) => permission);
            ...
            ensurePermissionSubset(
                actingUser,
                [...currentPermissions, ...requestedPermissions].map((permission: { code: string }) => permission.code),
            );
            ...
            if (input.permissionIds !== undefined) {
                await repository.replacePermissions(
                    roleId,
                    requestedPermissions.map((permission: { id: string }) => permission.id),
                );
            }

            const previousCodes = new Set(currentPermissions.map((permission: { code: string }) => permission.code));
            const requestedCodes = new Set(requestedPermissions.map((permission: { code: string }) => permission.code));
            await repository.writeAudit({
                actorUserId: actingUser.id,
                organizationId,
                entityId: roleId,
...
const deleteRole = async (
    ...
    await roleDb.withTransaction(async (repository: RoleTransactionRepository) => {
        ...
        ensurePermissionSubset(
            actingUser,
            role.permissions.map(({ permission }: { permission: { code: string } }) => permission.code),
        );
